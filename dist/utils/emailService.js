...
export const sendEmail = async (options) => {
    try {
        ...
        if (!process.env.GMAIL_APP_PASSWORD) {
            throw new Error("GMAIL_APP_PASSWORD is not configured. Generate an App Password from Gmail");
        }
...
export const sendBookingConfirmation = async (email, bookingDetails) => {
    const { movieTitle, showtime, auditorium, seats, totalPrice, bookingId } = bookingDetails;
    const html = `
    <!DOCTYPE html>
    <html>
    <head>
...
export const sendBookingCancellation = async (email, bookingDetails) => {
      ...
      <div class="container">
        ...
        ? `
          <div class="refund-info">
            ...
            <p>A refund of <strong>$${refundAmount.toFixed(2)}</strong> has been processed and will appear in your original payment method within 3-5 business days.</p>
          </div>
          `
        : ""}
