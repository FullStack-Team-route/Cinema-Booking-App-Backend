...
export const getAuditorium = async (req, res) => {
    try {
        const auditorium = await Auditorium.findById(req.params.id).populate("movies", "title poster category");
        if (!auditorium) {
...
export const updateAuditorium = async (req, res) => {
    try {
        ...
        if (location !== undefined)
            auditorium.location = location;
        if (isActive !== undefined)
            auditorium.isActive = isActive;
        // حفظ التحديثات
    ...
    try {
        ...
        if (auditorium.movies && auditorium.movies.length > 0) {
            ...
            if (moviesWithSlots.length > 0) {
                return res.status(400).json({
                    statusMsg: "fail",
                    message: "Cannot delete auditorium. There are upcoming shows scheduled in this auditorium",
...
export const manageAuditoriumMovies = async (req, res) => {
    try {
        ...
        if (action === "add") {
            // إضافة الفيلم للقاعة
            if (!auditorium.movies.includes(movieId)) {
                auditorium.movies.push(movieId);
                await auditorium.save();
            }
            // إضافة القاعة للفيلم (المفروض نضيفه هنا)
            if (!movie.auditoriums.includes(auditoriumId)) {
                movie.auditoriums.push(auditoriumId);
                await movie.save();
            }
        }
        else if (action === "remove") {
            // إزالة الفيلم من القاعة
            auditorium.movies = auditorium.movies.filter((id) => id.toString() !== movieId);
            await auditorium.save();
            // إزالة القاعة من الفيلم (المفروض نضيفه هنا)
            movie.auditoriums = movie.auditoriums.filter((id) => id.toString() !== auditoriumId);
            await movie.save();
        }
        res.status(200).json({
            statusMsg: "success",
